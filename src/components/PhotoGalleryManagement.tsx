...
export default function PhotoGalleryManagement() {
  ...
  const fetchData = () => {
    setIsLoading(true);
    fetch(`${API_URL}/api/photoGallery?t=${Date.now()}`)
      .then(res => res.json())
      .then(result => {
  ...
  try {
    ...
    if (mode === 'add') {
      // --- 情境一：批量新增 ---
      url = `${API_URL}/api/photoGallery/createPhotoGalleryBatch`; // 呼叫新的批量 API
      ...
      // --- 情境二：單一編輯 ---
      url = `${API_URL}/api/photoGallery/updatePhotoGallery`; // 呼叫原有的更新 API
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/photoGallery/deletePhotoGallery/${id}`, { method: 'DELETE' });
      if (res.ok) {
