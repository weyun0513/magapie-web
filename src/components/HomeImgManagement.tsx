...
export default function HomeImgManagement() {
  ...
  const [currentType, setCurrentType] = useState('ButtonPics');
  ...
  const handleSave = async () => {
    ...
    if (editingItem) {
      ...
      formData.append('image', editingItem.image);
      // 確保 type 也有傳給後端，或是從 editingItem 取得
    ...
    try {
      // 2. 動態判斷 API 路徑
      const isBanner = currentType === 'Banner';
      const action = editingItem ? 'update' : 'create';
      const category = isBanner ? 'Banner' : 'BttomImg';
      const url = `${API_URL}/api/home/${action}${category}`;
      ...
      const savedItem: ImgItem = result.data;
      // console.log("savedItem:", savedItem);
      const stateKey = savedItem.type === 'Banner' ? 'banner' : 'bottomPics';

      setHomeData(prev => {
        // console.log("當前的 homeData 內容:", prev);
        const newState = {
          ...prev,
          [stateKey]: editingItem
            ? (prev[stateKey] || []).map(i => (i.id === editingItem.id ? savedItem : i))
            : [...(prev[stateKey] || []), savedItem]
        };

        // console.log("即將更新的新物件結構:", newState);
  ...
  const handleDelete = async (item: ImgItem) => {
    const isConfirmed = window.confirm("確定要刪除這筆圖片嗎？");
    if (!isConfirmed) return;
    try {
      // 2. 發送刪除請求
      const response = await fetch(`${API_URL}/api/home/deleteBttomImg/${item.id}`, {
      ...
      if (item.type == "Banner") {
        setHomeData(prev => ({
          ...prev,
          bottomPics: prev.bottomPics.filter(i => i.id !== item.id)
  ...
  return (
    <div className="w-full p-6 bg-gray-50 ">
            ...
            {fixedBanners.map((item, index) => (
              <tr key={item?.id || `empty-banner-${index}`} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4">
                  {item ? (
                    <img src={`${API_URL}${item.image}`} className="w-12 h-12 rounded-lg object-cover border border-gray-200" />
                  ) : (
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    {item ? (
                      <>
                         <button onClick={() => { setCurrentType('Banner'); openModal(item); }}>編輯</button>
                        {/* <button onClick={() => handleDelete(item)} className="...">刪除</button> */}
                      </>
                    ) : (
                      <button onClick={() => openModal({ type: 'Banner' })} className="text-blue-600 hover:underline text-sm">新增資料</button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      <div className="flex items-center justify-between mb-6 mt-4">
        ...
        <button onClick={() =>{ setCurrentType('BottomPics'); openModal(null); }} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg flex items-center gap-2 transition-all shadow-md">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {homeData.bottomPics.map((item, index) => (
              <tr key={item?.id || `empty-banner-${index}`} className="...">
                <td className="px-6 py-4">

                  {item ? (
                    <img src={`${API_URL}${item.image}`} className="w-12 h-12 rounded-lg object-cover border border-gray-200" referrerPolicy="no-referrer" />
                  ) : (
                ...
                <td className="px-6 py-4 text-right">
                  ...
                  <div className="flex justify-end gap-2">
                    <button onClick={() => { setCurrentType('BottomPics'); openModal(item); }}>編輯</button>
